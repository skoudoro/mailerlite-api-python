...
def get_issues(project="skoudoro/mailerlite-api-python", state="closed",
               pulls=False):
...
def get_tags(project="skoudoro/mailerlite-api-python"):
    """Get a list of the tags from the Github API."""
    url = "https://api.github.com/repos/{0}/tags".format(project)
    return get_paged_request(url)


def fetch_basic_stats(project="skoudoro/mailerlite-api-python"):
...
def fetch_contributor_stats(project="skoudoro/mailerlite-api-python"):
...
def issues_closed_since(period=LAST_RELEASE,
                        project="skoudoro/mailerlite-api-python",
                        pulls=False):
...
def get_all_versions(ignore='', project="skoudoro/mailerlite-api-python"):
...
def username_to_fullname(all_authors):
    contributors_file = os.path.join(os.path.dirname(__file__), "..",
                                     CONTRIBUTORS_FILE)
